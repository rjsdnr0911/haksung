...
- [ ] 기능적 언락 상점 (Shop)
  ...
  - [ ] **Reroll** - 레벨업 선택지 재굴림
  - [ ] **Skip** - 레벨업 스킵 (나중에 선택)
  - [ ] **Banish** - 특정 Tome을 이번 런에서 제거
  - [ ] **Toggler** - 특정 무기/Tome 영구 비활성화/활성화
...
전체 진행률: 57% (4/7 phases)
```

---

## 🎯 다음 작업: Phase 5 - 메타 진행 시스템

### 개요
원작 Megabonk처럼 기능적 언락 시스템 구축 (스탯 업그레이드 없음)

### Phase 5 핵심 구현
1. **화폐 시스템** - 런에서 실버 획득, 누적 저장
2. **런 종료 화면** - 보상 요약, 실버 계산
3. **상점 시스템** - 무기/Tome 언락, 기능 구매
4. **빌드 컨트롤 도구**
   - Reroll (선택지 재굴림)
   - Skip (레벨업 유예)
   - Banish (Tome 제거)
   - Toggler (컨텐츠 On/Off)
5. **슬롯 확장** - 무기/Tome 슬롯 증가
6. **세이브 시스템** - LocalStorage로 진행 저장
...
### 다음 세션 목표
**Phase 5 시작**: 메타 진행 시스템 (화폐, 상점, 기능 언락, 빌드 컨트롤)
